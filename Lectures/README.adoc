...
== February 1: CPU overview

RAM (Random Access Memory):: The location of programs and/or data.
ALU (Arithmetic/Logic Unit):: Given two inputs, produce one output based on the opcode
Register file:: A small array of registers to read/write.
Control unit:: The underlying state machine that directs traffic among the ALU, register file, and RAM.

https://github.com/lawrancej/COMP1200-2016/blob/master/Circuits/cpu-16bit-isa.adoc[CPU instruction set]

== February 3: Programming the CPU

*Example:* An implementation of factorial.
...
Although the example CPU is limited, it is as capable as any other CPU.

. The instruction set (like most CPUs) requires data to reside in registers, including constants.
That is why we must load 1 into a register first: high-level constructs (such as increment) assume the literal 1.
It is also why we systematically replace meaningful variable names with registers.
. Assume that memory address `0x80` has the number we'll use to compute factorial.
`LOAD` is distinct from `MOV`, because `LOAD` copies data from RAM into a register, whereas `MOV` copies data from the instruction into the register.
. In this CPU (as in the ARM architecture), register 0 is a constant 0.
Thus, we can move data from one register to another by adding R0.
. Structured code is more readable because if the condition is true, we enter the block; otherwise, we jump past the block.
Spaghetti code (and machine code) is less readable because if the condition is true, we jump to some address.
Jumping to the next instruction achieves nothing, because the CPU will advance to the next instruction regardless.
Thus, we must invert the condition to achieve the effect of the structured code.
. When we reach the end of the loop, we jump back to the beginning.

== February 12: Memory hierarchy, displays

Memory speed and capacity are at odds with each other: fast CPU registers hold bytes, whereas slow hard drives hold terabytes.

* http://blog.codinghorror.com/the-infinite-space-between-words/[Memory hierarchy]
* https://www.google.com/search?q=a+light+nanosecond&ie=utf-8&oe=utf-8#q=+%28lightyear+%2F+1+year%29+in+feet+per+second[Light travels roughly 1 foot per nanosecond]
* https://en.wikipedia.org/wiki/Kibibyte[Know your SI prefixes for information]
* https://www.youtube.com/watch?v=Wiy_eHdj8kg[Hard drive teardown]

Displays are amazing.

* https://www.youtube.com/watch?v=jiejNAUwcQ8[LCD teardown]

High dynamic range displays are amazing-er (and are coming on the market soon)

* https://en.wikipedia.org/wiki/High-dynamic-range_imaging[HDR imaging]
* http://www.bit-tech.net/hardware/2005/10/04/brightside_hdr_edr/8[Two displays, side by side. Both are showing black. Spot the difference?]
* http://www.cnet.com/news/what-is-hdr-for-tvs-and-why-should-you-care/[High dynamic range displays]

== February 19: Encodings

* https://en.wikipedia.org/wiki/Prefix_code[Prefix code stuff]
* http://www.asciitable.com/[ASCII]
* https://en.wikipedia.org/wiki/UTF-8[Unicode]
* http://www.asciimation.co.nz/[etc.]
* https://xiph.org/video/vid1.shtml[Digital media primer for geeks]

== February 24: CPU stuff
...
== February 26: Modifying a CPU

Added memory-mapped output to a CPU.

Also: http://www.jakepoz.com/debugging-behind-the-iron-curtain/[Debugging behind the Iron Curtain]

== February 29: Modify a CPU more

Add memory-mapped input to a CPU.

== March 4: A need for speed
...
Bag of tricks to reduce latency
...
If you've ever used Java, JavaScript, VirtualBox, VMWare, or any emulator,
you've used a virtual machine.
A virtual machine implements an instruction set in software.
In a very real sense, hardware is just "petrified" software.

Peruse `Java/src/main/java/CPU.java` for a working Java implementation of the CPU
given in class.
